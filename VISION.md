...
A deliberate, research-heavy job search runs on one question, asked over and over: **does this role actually fit what I want next?** Not "am I qualified," but "is this worth my time, given my comp floor, the kind of work I want, the domains I'll and won't touch, and the deal-breakers I keep re-explaining to recruiters."
...
- **Jobs**: the full-screen home. Roles tracked by stage in a kanban or list view, each carrying its fit verdict, with a live keyword search across the board. Paste a job description (or a Greenhouse/Lever URL) to populate the fields; moving a job to *Applied* stamps the date, and moving it to *Closed* records why.
...
- **Fit scoring**: on demand, score any job against your criteria. The result is a two-axis verdict, *fit* (perfect / close / partial / miss) and recommended *action* (apply / wait / pass), with short, per-dimension reasoning, like a trusted recruiter's honest read. Fit is *derived* from a per-dimension rubric (role, domain, comp, and stack, each rated match / partial / gap), so the top "perfect" rank is a checkable judgment rather than a vibe: it lands only when every dimension matches and comp is stated above your floor. Scoring is deterministic, so the same inputs yield the same verdict. Action can diverge from fit (a great role with a red flag; a partial fit worth applying anyway), and when it does, the reasoning says why.
- **Calibration**: rate any verdict ("was this right?") and your correction is stored, then fed back as a few-shot anchor so future scores match your bar. Correcting a verdict also overrides that job's score immediately, with a persistent "you set this" marker. The scorer learns your standard from your own labeled judgments, not a fixed prompt.
...
It is local-first: your data is a JSON file you control. Export to back up or move devices, import (merge) to combine, or paste raw text to start populating. A bundled sample board lets a newcomer explore a populated app in one click before adding anything of their own. No account, no database, no server-side storage of your personal tracking data.

The one exception: shared, rate-limited AI endpoints (see Architecture) so anyone can try the paste-to-populate, fit-scoring, and résumé-seeding flows without their own API key. Only the pasted text passes through, transiently, and is never logged or stored.
...
- **Backend**: small Vercel serverless functions holding the Anthropic API key server-side, each a single tightly-scoped prompt, never open-ended chat:
  ...
  - `/api/score-fit`: personalized fit verdict from criteria + JD, fed your recent calibration corrections as anchors and run deterministically.
  - `/api/parse-resume`: turn a pasted résumé into Fit Criteria seed values.
- **Rate limiting**: a daily cap per browser for each task, backed by a best-effort server-side per-IP limiter, so the shared key can't be drained by one bad actor or a link going viral. A per-browser token tally (hidden by default) gives at-a-glance cost awareness; the Anthropic Console stays authoritative for real spend.
- **Storage**: none server-side. Export/import JSON to persist, move, or merge; paste raw text to start fresh.
- **Testing**: a Vitest unit suite runs in CI on every push and pull request, alongside a production build.
- **Repo**: GitHub.

## Current state

- **Built and in use**: Jobs (kanban + list, auto applied-date, keyword search, required close reasons), Fit Criteria editor, on-demand fit scoring (perfect / close / partial / miss with a per-dimension rubric, deterministic), the calibration loop, résumé seeding, Orgs, Contacts, paste-to-populate (incl. ATS URL), JSON export/import (merge), and a bundled sample board for first-run exploration. A Vitest suite runs in CI.
...
- Share-a-job: encode a single record into a URL for a self-contained, read-only view, consistent with the file-not-database principle. Helper functions exist in `store.js`; the UI isn't built yet.
...
- Daily rate-limit numbers are set conservatively for now (parse 50, score 25, résumé 50 per browser); watch real usage and adjust.
